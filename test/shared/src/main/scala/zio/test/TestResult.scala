...
case class TestResult private (private val arrow: TestArrow[Any, Boolean]) { self =>

  lazy val result: TestTrace[Boolean] = TestArrow.run(arrow, Right(()))

  lazy val failures: Option[TestTrace[Boolean]] = TestTrace.prune(result, negated = false)
  ...
  def setGenFailureDetails(details: GenFailureDetails): TestResult =
    TestResult(arrow.withGenFailureDetail(details))
}
